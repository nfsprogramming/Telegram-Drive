...
export function AuthWizard({ onLogin, isAddingAccount, onCancelAdd }: { onLogin: () => void, isAddingAccount?: boolean, onCancelAdd?: () => void }) {
    ...
    const saveAccountToStore = async (_phoneHint: string) => {
        try {
            // Get the verified user info from the actual session (not the user-typed phone)
            let verifiedPhone = _phoneHint;
            let verifiedId = sessionName; // fallback to timestamp
            let verifiedFirstName = '';
            try {
                const me = await invoke<{ id: number; phone: string; first_name: string }>('cmd_get_me');
                verifiedPhone = me.phone || _phoneHint;
                verifiedId = me.id.toString();
                verifiedFirstName = me.first_name || '';
            } catch (e) {
                console.warn('[saveAccountToStore] cmd_get_me failed, using phone hint:', e);
            }

            const store = await load('config.json');
            const accounts = await store.get<any[]>('accounts') || [];

            // Deduplicate by Telegram user ID (most reliable key)
            const existing = accounts.find(a => a.id === verifiedId || a.session_name === sessionName);
            if (!existing) {
                accounts.push({
                    id: verifiedId,
                    phone: verifiedPhone,
                ...
                // Update in case it changed or was wrong
                existing.phone = verifiedPhone;
                existing.first_name = verifiedFirstName;
            }
            await store.set('accounts', accounts);
            await store.set('active_account_id', verifiedId);
            await store.save();
        } catch (err) {
    ...
    const handleQrLogin = async () => {
        ...
        try {
            ...
            if (url === "__authorized__") {
                await saveAccountToStore("QR Login (Refresh)");
                onLogin();
                return;
            }
            ...
            try {
                ...
                if (res.success) {
                    ...
                    if (res.next_step === "password") {
                        ...
                        await saveAccountToStore("QR Session");
                        onLogin();
                    }
                }
    ...
    const handleCodeSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.success) {
                await saveAccountToStore(phone || sessionName);
                onLogin();
    ...
    return (
        <div className="h-full w-full auth-gradient flex items-center justify-center p-6 relative">
            {isAddingAccount && onCancelAdd && (
                <button 
                    onClick={onCancelAdd} 
                    className="absolute top-4 left-4 p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors z-10 text-white"
                >
                    <X className="w-5 h-5" />
                </button>
            )}
            ...
            >
                ...
                <div className="mt-8 pt-4 border-t border-white/5 text-center">
                    ...
                    >
                        <Heart className="w-3.5 h-3.5 text-blue-500/80" />
                        About Developer
                    </button>
                </div>
            </motion.div>
                        ...
                        >
                            ...
                            <div className="space-y-4 text-center">
                                <p className="text-sm text-telegram-subtext mb-6">
                                    Telegram Drive is proudly created and maintained by:
                                </p>

                                <div className="space-y-2 mb-6">
                                    <h3 className="text-lg font-bold text-white">NFS Programming</h3>
                                    <p className="text-sm text-gray-300">Author: Nifras</p>
                                </div>

                                <a href="mailto:mohammed.nifras.000555@gmail.com" className="w-full bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 font-semibold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors">
                                    <ExternalLink className="w-4 h-4" />
                                    Contact Support
                                </a>
                            </div>
