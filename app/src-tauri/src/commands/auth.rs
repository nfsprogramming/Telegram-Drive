...
pub async fn ensure_client_initialized(
    ...
    let session_filename = format!("telegram_{}.session", session_name);
...
pub async fn cmd_connect(
    ...
    log::info!("cmd_connect: session_name={}", session_name);

    // ALWAYS drop the existing client and peer cache before connecting.
    // This guarantees we load the correct session file every time, even if
    // the session name appears unchanged (e.g., same Rust process after page reload).
    {
        *state.client.lock().await = None;
    }
    crate::commands::utils::clear_peer_cache(&state.peer_cache).await;
    *state.active_session.lock().await = Some(session_name.clone());

    // Store API ID for reconnect fallback
    *state.api_id.lock().await = Some(api_id);
    ensure_client_initialized(&app_handle, &state, api_id, &session_name).await?;
    log::info!("cmd_connect: successfully connected with session={}", session_name);
    Ok(true)
}
...
pub async fn cmd_check_connection(
    ...
    let session_name_opt = state.active_session.lock().await.clone();
    if let (Some(api_id), Some(session_name)) = (api_id_opt, session_name_opt) {
        // Force re-init: Clear old client first to ensure fresh pool
        *state.client.lock().await = None;
        
        match ensure_client_initialized(&app_handle, &state, api_id, &session_name).await {
            Ok(c) => {
                // Double check
...
pub async fn cmd_logout(
    ...
    *state.api_id.lock().await = None;
    ...
    let session_name_opt = state.active_session.lock().await.take();
...
pub async fn cmd_auth_request_code(
    ...
    let mut current_session_guard = state.active_session.lock().await;
    if *current_session_guard != Some(session_name.clone()) {
        *state.client.lock().await = None;
        crate::commands::utils::clear_peer_cache(&state.peer_cache).await;
    }
    *current_session_guard = Some(session_name.clone());
    drop(current_session_guard);

    // Store API ID
    *state.api_id.lock().await = Some(api_id);

    let client_handle = ensure_client_initialized(&app_handle, &state, api_id, &session_name).await?;
...
pub async fn cmd_auth_qr_login(
    ...
    *state.api_id.lock().await = Some(api_id);
    ...
    let client = ensure_client_initialized(&app_handle, &state, api_id, &session_name).await?;
...
pub async fn cmd_get_me(
    ...
    let (phone, username, first_name) = match me.raw {
        tl::enums::User::User(u) => {
            (u.phone.unwrap_or_default(), u.username, u.first_name)
        },
        _ => (String::new(), None, None),
    };

    log::info!("cmd_get_me: id={} phone={}", id, phone);

    Ok(crate::models::UserInfo {
        id,
        phone: if phone.starts_with('+') { phone } else { format!("+{}", phone) },
        username,
