...
#[tauri::command]
pub async fn cmd_clear_peer_cache(state: State<'_, crate::TelegramState>) -> Result<(), String> {
    // Drop the current client so that a fresh one is created on next connect
    *state.client.lock().await = None;
    clear_peer_cache(&state.peer_cache).await;
    log::info!("Peer cache and client cleared on account switch.");
    Ok(())
}
